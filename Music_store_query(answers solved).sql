...
select artist.artist_id,artist.name, count(track.track_id) as track_count from artist 
...
select track.name, track.milliseconds as track_length from track
...
select customer.customer_id, customer.first_name, customer.last_name, bsa.artist_name as Artist_name,  
...
with most_spent_country as
	(select customer.customer_id, customer.first_name, customer.last_name, 
	sum(invoice.total) as Amount_spent, invoice.billing_country,
	row_number() over(partition by invoice.billing_country order by sum(invoice.total) 
	desc) as row_no
	from invoice
	join customer on invoice.customer_id=customer.customer_id
	group by 1,2,3,5
	order by 5 asc,4 desc)
select * from most_spent_country where row_no <=1
